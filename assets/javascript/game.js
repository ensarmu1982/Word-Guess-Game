// Note: COPIED AND MODIFIED FROM CLASS ACTIVITY WORD GUESSING GAME
//GLOBAL VARIABLES (accessible by all functions)
// ==================================================================================================

// Array of Word Options (all lowercase).
var wordsList = ["algeria", "angola", "benin", "botswana", "burkina faso", "burundi", "cabo verde", "cameroon", 
...
"seychelles", "sierra leone", "somalia", "south africa", "south sudan", "sudan", "eswatini", "tanzania", "togo",
...
function checkLetters(letter) {
  ...
  //for wrong guesses
  else { //check if letter exists in the wrongGuesses array before subtracting number of guesses 

      //If letter is wrongly gussed previously DO NOT subtract (if statement)  
      for (var n = 0; n < wrongGuesses.length; n++){
        var m = wrongGuesses.indexOf(letter);  
        if (m >= 0){
          console.log("letter exists index of: " + m);
          numGuesses = numGuesses;
          return m;
        }
      }

      //if letter not previously gussesed update wrong guesses
...
function roundComplete() {
  ...
  //track winning and losing here
  if (lettersInChosenWord.toString() === blanksAndSuccesses.toString()) {

    // Add to the win counter
    winCounter++;

    //show the win word and update county to guess to you guessed right
    // (meaning that we guessed all the letters to match the solution)...
    function winner (){
      var i = lettersInChosenWord.toLocaleString;
  ...
  // If we've run out of guesses
    ...
    // Loss Restart the game immediatly
    startGame();
  }
}
